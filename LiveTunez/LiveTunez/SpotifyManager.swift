...
class SpotifyManager: ObservableObject {
    ...
    private let clientID = "a77e6fd95a4c4b31b09ff2d3370e3e85"
    private let clientSecret = "9ba34377af114b40bc66442b51ad3e43"
    private let redirectURI = "livetunez://callback"
    ...
    func authorize(completion: @escaping (Bool) -> Void) {
        validateAccessToken { [weak self] isValid in
            print("Validating access token, is valid: \(isValid)")
            if !isValid {
                ...
                UIApplication.shared.open(url) { success in
                    print("Opening URL for authorization, success: \(success)")
                    completion(success)
                }
            } else {
                completion(true)
    ...
    func handleRedirectURL(_ url: URL, completion: @escaping (Bool) -> Void) {
        ...
        print("Code extracted: \(code), exchanging for token...")
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            if let json = try? JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
               ...
               let refreshToken = json["refresh_token"] as? String {
                ...
                print("Refresh Token stored: \(refreshToken)")
                completion(true)
            } else {
                completion(false)
            }
    ...
    func searchTrackIDs(songNames: [String], artistName: String, completion: @escaping ([String]) -> Void) {
        ...
        group.notify(queue: .main) {
            print("All track IDs fetched: \(trackIDs)")
            completion(trackIDs)
        }
    }
    ...
    func validateAndAuthorize(completion: @escaping (Bool) -> Void) {
        guard isInitializationComplete else {
            // If initialization is not complete, defer the authorization check.
        ...
        validateAccessToken { isValid in
            if isValid {
                self.isAuthenticated = true
                completion(true)
            } else {
    ...
    private func loadAccessTokenFromFirebase(completion: @escaping (Bool) -> Void) {
        print("Loading access token from Firebase...")
        db.collection("devices").document(deviceID).getDocument { snapshot, error in
            if let error = error {
            ...
            guard let data = snapshot?.data(),
                  ...
                  let refreshToken = data["refreshToken"] as? String else {
                print("No access token or refresh token found in Firebase")
                completion(false)
                return
            }

            self.accessToken = accessToken
            print("Access token loaded from Firebase: \(accessToken), Refresh token: \(refreshToken)")
            self.validateAccessToken { isValid in
                print("Post-load validation result: \(isValid)")
                completion(isValid)
            }
        }
    }
