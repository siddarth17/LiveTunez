...
class ConcertDetailViewModel: ObservableObject {
    ...
    private func fetchWeatherData() {
        let urlString = "https://api.openweathermap.org/data/3.0/onecall?lat=\(latitude)&lon=\(longitude)&exclude=minutely,hourly,daily,alerts&appid=ba1473211a6a9b35c8911067218575b2&units=metric"
    ...
    func fetchSetlistData() {
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            do {
                ...
                print("Decoded setlist data successfully")
                DispatchQueue.main.async {
    ...
    func importSetlistToSpotify() {
        print("Attempting to import setlist to Spotify...")
        
        guard SpotifyManager.shared.isAuthenticated, let accessToken = SpotifyManager.shared.accessToken else {
            print("User is not authenticated or access token is not available. Need to authenticate first.")
            return
        }
        
        print("User is authenticated, Access Token: \(accessToken)")
        ...
        print("Import to Spotify function completed.")
    }
